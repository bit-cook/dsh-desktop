...
const PATCH = 'patches/@deepseek-ai+dsh-client-ui-conversation+0.1.5-rc.2.patch'
...
describe('composer dock meter placement', () => {
  ...
  it('renders the composer "+" and attach buttons without a resting background', async () => {
    const patch = await readFile(PATCH, 'utf8')
    const marker = '+\t\t\t\t\t\t\t\t\t\t\t\t"data-dsh-composer-add": "",'
    expect(patch.split(marker).length - 1).toBe(2)
